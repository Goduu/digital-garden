  ...
  return (
    <div className="max-w-sm mx-auto" ref={componentRef}>
      <div className="flex relative">
        ...
        <div className={`z-10 ${!open && "hidden"} absolute bottom-full sm:top-full`}>
          <ul className=" text-sm bg-white divide-y divide-slate-100 dark:divide-slate-500 rounded-s-lg shadow dark:bg-slate-700" aria-label="language-button">
            {locales.map((language) => (
              <li key={language}>
                <button
                  type="button"
                  aria-label={language}
                  className="inline-flex w-full rounded-s-lg px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-600 dark:hover:text-white"
